...
class ClubsEnv(gym.Env):  # type: ignore
    ...
    def __init__(
        ...
        # Handle empty community cards case for gymnasium compatibility
        if comm_card_numb > 0:
            community_cards_space = spaces.Tuple((card_space,) * comm_card_numb)
        else:
            # Use a dummy space when there are no community cards
            community_cards_space = spaces.Tuple((spaces.Discrete(1),))
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "community_cards": community_cards_space,
    ...
    def step(  # type: ignore
        self, bet: int
    ) -> Tuple[clubs.poker.engine.ObservationDict, List[int], List[bool], bool, dict]:
    ...
    def reset(  # type: ignore
        self, seed=None, options=None, reset_button: bool = False, reset_stacks: bool = False
    ) -> tuple:
        if seed is not None:
        ...
        if options is not None:
            reset_button = options.get('reset_button', reset_button)
            reset_stacks = options.get('reset_stacks', reset_stacks)
